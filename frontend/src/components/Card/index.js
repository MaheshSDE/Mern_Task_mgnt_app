...
const Card = (props) => {
    ...
    const handleCompleteTask=async(id)=>{
        try {
            await axios.put(`http://localhost:4001/updateCompleteTask/${id}`,{},{headers});
        } catch (error) {
            console.log(error)
        }
    }
    ...
    const handleImportant=async(id)=>{
        try {
           const response= await axios.put(`http://localhost:4001/updateImportantTask/${id}`,{},{headers});
    ...
    const deleteTask=async(id)=>{
        try {
            const response=await axios.delete(`http://localhost:4001/deleteTask/${id}`,{headers})
            console.log(response);
        } catch (error) {
            console.log(error);
        }
